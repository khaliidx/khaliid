...
function getLocation() {
	...
	function succesGeo(position) {
		...
		directionsDisplay.setMap(map); //DirectionsRenderer pour savoir ou il va dessiner, il faut setMap, pour lui donner la map ou il va dessiner
		var laFac = new google.maps.LatLng(34.007807, -6.838259); //La fac coordinates
		var request = { //Led details des chemin qu'on vaut DirectionsService trouve pour nous
		...
		directionsService.route(request, function(response, status) {
			...
			// d'abord checker le status de la recherche de notre chemin, il existe ou nn, ou s'il y a un problem dans les parametre de requete
			if (status == google.maps.DirectionsStatus.OK) { //s'il y a aucun prbleme
				...
				var markerFac = new google.maps.Marker({ //facultatuf, pour ajouter un marker pour la fac, il y en a déjà un
					position: laFac,
					map: map,
